...
def register():
    ...
    try:
        ...
        flash("Registration successful! You are now logged in.", "success")  # Dodaj komunikat
...
def logout():
    ...
    flash("Zostałeś wylogowany.", "success")  # Dodaj komunikat
...
def upload_and_predict():
    ...
    for file in files:
        if file.filename != '':
            ...
            #  Jeśli nie wykryto twarzy, zwróć błąd
            if not results:
                flash('Zostało wgrane złe zdjęcie – brak wykrytej twarzy na jednym ze zdjęć', 'danger')
                os.remove(file_path)  # Usuń niepoprawne zdjęcie
                return redirect(url_for('index'))
    ...
    flash('Nie przesłano żadnych plików', 'warning')
    return redirect(url_for('index'))
...
def download_zip():
    try:
        ...
        if not files_to_zip or not username:
            return jsonify({'success': False, 'error': 'Brak plików do zapisania lub użytkownika'}), 400
        ...
        with zipfile.ZipFile(memory_file, 'w') as zipf:
            for file_info in files_to_zip:
                ...
                # Loguj ścieżki i sprawdzaj, czy plik istnieje
...
def skip_image():
    ...
    original_path = os.path.join(user_folder, 'photos', filename)
    skipped_folder = os.path.join(user_folder, 'skipped')

    os.makedirs(skipped_folder, exist_ok=True)

    try:
        shutil.move(original_path, os.path.join(skipped_folder, filename))
        return jsonify({"success": True})
    except Exception as e:
        return jsonify({"success": False, "error": str(e)}), 500
